...
def add_scholar_links_to_references(md_text):
    lines = md_text.splitlines()
    new_lines = []
    in_refs = False
    for line in lines:
        ...
        if in_refs:
            ...
            new_lines.append(f"[{line.strip()}]({scholar_url})")
        else:
            new_lines.append(line)
    return "\n".join(new_lines)
...
if st.button("🚀 Generate Article", type="primary"):
    if not api_key:
        ...
        if md:
            ...
            st.divider()
            st.download_button("⬇️ Download Markdown", data=md_with_links, file_name="article.md", mime="text/markdown")

            try:
                docx_bytes = export_docx(md_with_links)
            ...
            try:
                pdf_bytes = export_pdf_basic(md_with_links)
